...
Telegram rejects unless it is a well-formed http(s) URL - so a commit validates
the scheme and refuses anything else rather than storing a value that would
later blow up when shown.
"""
...
class WebsiteService:
    ...
    def commit_pending_url(self, telegram_id: int) -> tuple[str | None, UsersToState]:
        """Persist the staged URL if it is a valid http(s) link. Returns the saved
        URL, or None if the staged value was rejected (the staging field is cleared
        either way)."""
        user_to_state, pending_url = self._take_pending(telegram_id)
        pending_url = pending_url.strip()
        if not self._is_valid_url(pending_url):
