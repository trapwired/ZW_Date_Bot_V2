...
def _trainer_texts(bot, services):
    trainer_chat_id = services["telegram_service"].trainers_games[0]
...
async def test_fires_when_at_most_max_players_stay_available(services, data_access, bot, game, player_ids):
    ...
    texts = _trainer_texts(bot, services)
...
async def test_silent_while_more_than_max_players_available(services, data_access, bot, game, player_ids):
    ...
    assert _trainer_texts(bot, services) == []


async def test_yes_update_never_fires(services, data_access, bot, game, player_ids):
